/***********************************************************************
 * $Id:: s1l_cmds_core.c 871 2008-07-07 22:36:49Z wellsk               $
 *
...
static CMD_ROUTE_T core_peek_cmd =
{
	"peek",
	cmd_peek,
	"Displays a byte, half-word, or word value at an address",
	"peek <hex address><1, 2, or 4 bytes>",
	cmd_peek_plist,
	NULL
};

/* Poke command */
BOOL_32 cmd_poke(void);
...
static CMD_ROUTE_T core_poke_cmd =
{
	"poke",
	cmd_poke,
	"Places a byte, half-word, or word value at an address",
	"poke [hex address][hex value][1, 2, or 4 bytes]",
	cmd_poke_plist,
	NULL
};

/* Dump command */
BOOL_32 cmd_dump(void);
...
static CMD_ROUTE_T core_dump_cmd =
{
	...
	"dump <hex address><values to dump><1, 2, or 4 bytes>",
	cmd_dump_plist,
	NULL
};

/* Fill command */
BOOL_32 cmd_fill(void);
...
static CMD_ROUTE_T core_fill_cmd =
{
	...
	"fill [hex address][bytes to fill(hex)][hex value][1, 2, or 4 bytes]",
	cmd_fill_plist,
	NULL
};

/* Memory test command */
BOOL_32 cmd_memtst(void);
...
static CMD_ROUTE_T core_memtst_cmd =
{
	"memtst",
	cmd_memtst,
	"Performs 1 or more memory tests",
...
static CMD_ROUTE_T core_bwtest_cmd =
{
	"bwtest",
	cmd_bwtest,
	"Performs a bandwidth test on a range of memory",
	"bwtest [source hex address][destination hex address][bytes][loops]",
	cmd_bwtest_plist,
	NULL
};

/* comp command */
BOOL_32 cmd_comp(void);
...
static CMD_ROUTE_T core_comp_cmd =
{
	"comp",
	cmd_comp,
	"Performs a bytewise data comparison between 2 data regions",
	"comp [hex address 1][hex address 2][bytes]",
	cmd_comp_plist,
	NULL
};

/* copy command */
BOOL_32 cmd_copy(void);
...
static CMD_ROUTE_T core_copy_cmd =
{
	"copy",
	cmd_copy,
	"Performs a bytewise data copy between 2 data regions",
	"copy [source hex address][destination hex address][bytes]",
	cmd_copy_plist,
	NULL
};

/* baud command */
BOOL_32 cmd_baud(void);
...
static CMD_ROUTE_T core_baud_cmd =
{
	"baud",
	cmd_baud,
	"Changes the system terminal baud rate",
...
static CMD_ROUTE_T core_prompt_cmd =
{
	"prompt",
	cmd_prompt,
	"Changes the system prompt and autoboot timeout",
	"prompt [name][timeout(secs)]",
	cmd_prompt_plist,
	NULL
};
...
static CMD_ROUTE_T core_info_cmd =
{
	"info",
	cmd_info,
	"Displays the system information and configuration",
	"info",
	cmd_info_plist,
	NULL
};
...
/* Core group */
static GROUP_LIST_T core_group =
{
	"core", /* Core group */
	"S1L core system command group",
	NULL,
	NULL
};
...
BOOL_32 cmd_info(void) {
	...
	if (sysinfo.nandgood == TRUE) 
	{
		term_dat_out_crlf(present_msg);
		term_dat_out(numblks_msg);
		str_makedec(str, sysinfo.nandgeom.blocks);
		term_dat_out_crlf(str);
		term_dat_out(pgsblk_msg);
		str_makedec(str, sysinfo.nandgeom.sectors_per_block);
		term_dat_out_crlf(str);
		term_dat_out(pgsbyt_msg);
		str_makedec(str, sysinfo.nandgeom.data_bytes_per_sector);
		term_dat_out_crlf(str);
		tmp = sysinfo.nandgeom.blocks *
			sysinfo.nandgeom.sectors_per_block *
