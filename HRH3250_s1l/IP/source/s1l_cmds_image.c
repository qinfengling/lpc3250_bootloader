/***********************************************************************
 * $Id:: s1l_cmds_image.c 871 2008-07-07 22:36:49Z wellsk              $
 *
...
static CMD_ROUTE_T core_ls_cmd =
{
	"ls",
	cmd_ls,
	"Displays files in the root directory of the block device",
	"ls",
	cmd_ls_plist,
	NULL
};
...
static CMD_ROUTE_T core_load_cmd =
{
	"load",
	cmd_load,
	"Loads a file into memory from a source",
	"load [source <filename>][type]<load address><exec address>",
	cmd_load_plist,
	NULL
};
...
static CMD_ROUTE_T core_exec_cmd =
{
	"exec",
	cmd_exec,
	"Executes an image loaded in memory or at an address",
...
static CMD_ROUTE_T core_aboot_cmd =
{
	"aboot",
	cmd_aboot,
	"Sets up the board autoboot configuration",
	"aboot [source <filename>][type]<load address><exec address>",
	cmd_aboot_plist,
	NULL
};
...
static CMD_ROUTE_T core_boot_cmd =
{
	"boot",
	cmd_boot,
	"Boots with board with the current autoboot configuration",
	"boot",
	cmd_boot_plist,
	NULL
};
...
/* Image group */
static GROUP_LIST_T image_group =
{
	"image", /* Image group */
	"S1L image processing command group",
	NULL,
	NULL
};
