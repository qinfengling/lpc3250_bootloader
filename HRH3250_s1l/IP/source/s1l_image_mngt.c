/***********************************************************************
 * $Id:: s1l_image_mngt.c 877 2008-07-08 21:20:50Z wellsk              $
 *
	...
	while (bytes > 0) 
	{
		...
		if (checkblk == FALSE)
		{
			...
			if (lefttoread == 0) 
			{
				...
				if (curpage >= sysinfo.nandgeom.sectors_per_block) 
				{
				...
				else
				{
					...
					flash_read_sector(sector, secdat);
					lefttoread = sysinfo.nandgeom.data_bytes_per_sector;
					cindex = 0;
					curpage++;
				}
			}
...
BOOL_32 flash_image_save(void) 
{
	...
	while (nblks > 0) 
	{
		...
		{
			flash_erase_block(ffblock, FALSE);
			nblks--;
		}
	...
	if (mem_to_nand(sector, (UNS_8 *) sysinfo.lfile.loadaddr,
		(numsecs * sysinfo.nandgeom.data_bytes_per_sector), FALSE) == FALSE)
	{
...
BOOL_32 flash_image_load(void) 
{
	...
	if (nand_to_mem(sector, (UNS_8 *) syscfg.fsave.loadaddr,
		(syscfg.fsave.secs_used * sysinfo.nandgeom.data_bytes_per_sector),
		FALSE) == FALSE)
	{
 ...
 *     forcewrite      : TRUE to force write of bad blocks
 *
 * Outputs: None
 *
 * Returns: Nothing
					...
					BOOL_32 forcewrite)
{
	...
	block = starting_sector / sysinfo.nandgeom.sectors_per_block;
	page = starting_sector -
		(block * sysinfo.nandgeom.sectors_per_block);

	/* Align bytes to a sector boundary */
	wbytes = bytes & ~(sysinfo.nandgeom.data_bytes_per_sector - 1);
	if (wbytes < bytes)
	{
		bytes = wbytes + sysinfo.nandgeom.data_bytes_per_sector;
	}
	...
	while (bytes > 0)
	{
		...
		else
		{
			...
			/* Write sector data */
			if (flash_write_sector(sector, p8) == FALSE)
			{
			...
			else
			{
				...
				bytes -= sysinfo.nandgeom.data_bytes_per_sector;
			}

			/* Next page and block */
			page++;
			if (page >= sysinfo.nandgeom.sectors_per_block)
			{
				page = 0;
				block++;
				blkchk = !forcewrite;
 ...
 *     forcewrite      : TRUE to force read of bad blocks
 *
 * Outputs: None
 *
 * Returns: Nothing
					...
					BOOL_32 forcebad)
{
	...
	while (bytes > 0)
	{
		...
		else
		{
			...
			/* Write sector data */
			if (flash_read_sector(sector, secdat) == FALSE)
			{
			...
			{
				memcpy(p8, secdat, bytes);
				p8 += sysinfo.nandgeom.data_bytes_per_sector;
				if (bytes >= sysinfo.nandgeom.data_bytes_per_sector)
				{
					bytes -= sysinfo.nandgeom.data_bytes_per_sector;
				}
				else
