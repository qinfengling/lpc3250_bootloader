/***********************************************************************
 * $Id:: lpc32xx_uart_driver.c 1094 2008-08-18 22:19:36Z wellsk        $
 *
                 ...
                 INT_32 arg)
{
  ...
  if (uartnum >= 0)
  {
    ...
    if (uartdat[uartnum].uart_init == FALSE)
    {
      ...
      tmp = UARTCNTL->clkmode & UART_CLKMODE_MASK(uartnum + 1);
      UARTCNTL->clkmode = (tmp |
        UART_CLKMODE_LOAD(UART_CLKMODE_AUTO, (uartnum + 1)));
...
STATUS uart_close(INT_32 devid)
{
  ...
  if (puart->uart_init == TRUE)
  {
    ...
    tmp = UARTCNTL->clkmode & UART_CLKMODE_MASK(puart->uartnum + 1);
    UARTCNTL->clkmode = (tmp |
        UART_CLKMODE_LOAD(UART_CLKMODE_OFF, (puart->uartnum + 1)));
