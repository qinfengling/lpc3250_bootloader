/***********************************************************************
 * $Id:: lpc32xx_i2c_driver.c 1076 2008-08-17 00:24:08Z stefanovicz    $
 *
...
STATUS i2c_ioctl(INT_32 devid,
                 ...
                 INT_32 arg)
{
  volatile UNS_32 i, tmp, high, low;
  UNS_32 i2c_id = 9999;
      ...
      switch (cmd)
      {
        case I2C_SETUP:
          ...
          else
          {
            ...
            device->regptr->i2c_clk_lo = 
                (low * (tmp / (high + low))) / param->rate;
            device->regptr->i2c_clk_hi = 
                (high * (tmp / (high + low))) / param->rate;
          }
