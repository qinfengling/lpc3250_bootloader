/***********************************************************************
 * $Id:: lpc32xx_spi_driver.c 1107 2008-08-21 20:50:46Z stefanovicz    $
 *
    ...
    switch (cmd)
    {
      ...
      case SPI_DRIVE_SSEL:
        ...
        else if (spidrvdat->thisdev == 1)
        {
          if (arg == 0)
          {   //spi2_ssel = 0
            GPIO->p3_outp_clr = OUTP_STATE_GPIO(4);
          }
          else
          {   //spi2_ssel = 1
            GPIO->p3_outp_set = OUTP_STATE_GPIO(4);
          }
          //GPIO_04 is an output; select GPIO_04
          GPIO->p2_dir_set = OUTP_STATE_GPIO(4);
